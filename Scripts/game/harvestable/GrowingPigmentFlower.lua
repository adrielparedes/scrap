...
local GrowTickTime = DAYCYCLE_TIME_TICKS * 0.5 -- CAMBIE ACA
...
function GrowingPigmentFlower.sv_updateTicks( self, ticks )
	if not self.sv.regrown and sm.exists( self.harvestable ) then
		...
		if growFraction >= 1.0 then
			sm.harvestable.create( hvs_farmables_pigmentflower, self.harvestable.worldPosition, self.harvestable.worldRotation )
