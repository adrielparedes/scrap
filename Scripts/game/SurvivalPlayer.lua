...
SurvivalPlayer.RESET_MAP_MOD = false
SurvivalPlayer.REMOVE_MAP_MOD = false
SurvivalPlayer.OverWorld = nil
...
function SetOverWorld( world )
	SurvivalPlayer.OverWorld = world
end
...
function SurvivalPlayer.reset_map_data( self )
	SurvivalPlayer.RESET_MAP_MOD = true
end

function SurvivalPlayer.remove_map_data( self )
	SurvivalPlayer.REMOVE_MAP_MOD = true
end

function SurvivalPlayer.sv_reset_map_data( self )
	local UUID = sm.uuid.new( "2aa8cb2c-ddd4-4554-9019-5a1c34e2d196" )
	for k,v in pairs(sm.body.getAllBodies()) do
		for k,v in pairs(sm.body.getShapes(v)) do
			if UUID == v:getShapeUuid() then
				sm.shape.destroyShape(v,0)
			end 
		end
	end
	self.network:sendToClients( "client_showMessage", "#ffae00MAP MOD RESET, SAVING DATA...#ffffff" )
end

function SurvivalPlayer.sv_remove_map_data( self )
	self.network:sendToClients( "client_showMessage", "#ffae00CHECKING WORLD FOR MAP PARTS...#ffffff" )
	local UUID = { sm.uuid.new( "2aa8cb2c-ddd4-4554-9019-5a1c34e2d196" ), sm.uuid.new( "d0afb527-e786-4a22-a014-d847900458a7" ), sm.uuid.new( "f08d772f-9851-47b4-83ef-6da7e7cba8cb" ), sm.uuid.new( "448f7c93-699d-47b4-83ef-dbc78a1f9636" ) }
	for k,v in pairs(sm.body.getAllBodies()) do
		for k,v in pairs(sm.body.getShapes(v)) do
			if isAnyOf(v:getShapeUuid(), UUID) then
				sm.shape.destroyShape(v,0)
			end 
		end
	end
	self.network:sendToClients( "client_showMessage", "#03fc20DELETED CRITICAL MAP PARTS FROM WORLD\n#ffae00CHECKING PLAYER INVENTORY...#ffffff" )
	for k,v in pairs(sm.player.getAllPlayers()) do
		for l,m in pairs(UUID) do
			for g,o in pairs(sm.container.quantity(v:getInventory())) do
				local tool = sm.container.getItem(v:getInventory(), g)
				if tool ~= nil then
					if tool.uuid == sm.uuid.new("448f7c93-699d-47b4-83ef-dbc78a1f9636") then
						sm.container.beginTransaction()
						sm.container.setItem(v:getInventory(), g, sm.uuid.new("00000000-0000-0000-0000-000000000000"), 0)
						sm.container.endTransaction()
					end
				end
			end
			local Container = v:getInventory()
			sm.container.beginTransaction()
			local count = sm.container.totalQuantity(Container, m)
			sm.container.spend(Container, m, count)
			sm.container.endTransaction()
		end
	end
	--#ff0000
	self.network:sendToClients( "client_showMessage", "#03fc20DELETED CRITICAL ITEMS FROM ALL PLAYERS INVENTORY\n#ffae00SEARCHING CONTAINERS...#ffffff" )

	for k,v in pairs(sm.body.getAllBodies()) do
		for k,v in pairs(sm.body.getShapes(v)) do
			if v:getInteractable() ~= nil then
				if v:getInteractable():getContainer() ~= nil then
					for l,m in pairs(UUID) do
						local Container = v:getInteractable():getContainer()
						for g,o in pairs(sm.container.quantity(Container)) do
							local tool = sm.container.getItem(Container, g)
							if tool ~= nil then
								if tool.uuid == sm.uuid.new("448f7c93-699d-47b4-83ef-dbc78a1f9636") then
									sm.container.beginTransaction()
									sm.container.setItem(Container, g, sm.uuid.new("00000000-0000-0000-0000-000000000000"), 0)
									sm.container.endTransaction()
								end
							end
						end
	...
	self.network:sendToClients( "client_showMessage", "#03fc20DELETED CRITICAL ITEMS FROM CONTAINERS\n#9900ffFINALIZING REMOVAL...\n#ffae00Please Wait, This Will Take A Few Moments#ffffff" )
	local map = Map()
	map:SetCanSave(false)
end


function SurvivalPlayer.client_showMessage( self, message )
...
function SurvivalPlayer.cl_localPlayerUpdate( self, dt )
	...
	if self.cl.hud then
		...
		if character then
			...
			if SurvivalPlayer.RESET_MAP_MOD then
				SurvivalPlayer.RESET_MAP_MOD = false
				self.network:sendToServer("sv_reset_map_data")
			end

			if SurvivalPlayer.REMOVE_MAP_MOD then
				SurvivalPlayer.REMOVE_MAP_MOD = false
				self.network:sendToServer("sv_remove_map_data")
			end
			if SurvivalPlayer.IsOpen then
				if sm.localPlayer.getPlayer():getCharacter():getWorld() == SurvivalPlayer.OverWorld then
					self.cl.hud:setImage("COMPASS_BG", "$SURVIVAL_DATA/Scripts/WayPoints/camera-hud0.png")
					self.cl.hud:setText( "COMPASS_zoom", "#0000ffZoom#ffffff\n"..ScrollZoom()-2 )
					local dist = "inf"
					if GetPlayerData(sm.localPlayer.getPlayer():getName()) ~= nil then 
					dist = math.pow(GetPlayerData(sm.localPlayer.getPlayer():getName()).data.maxViewDist, 2)*100 
						if GetPlayerData(sm.localPlayer.getPlayer():getName()).data.maxViewDist == 16 then
							dist = "inf"
						end
						if GetPlayerData(sm.localPlayer.getPlayer():getName()).data.maxViewDist == 0 then
							dist = "Invisible"
						end
					end
					self.cl.hud:setText( "COMPASS_limit", "#0000ffWayPoint View Dist#ffffff\n"..dist )
				else
					self.cl.hud:setImage("COMPASS_BG", "$SURVIVAL_DATA/Scripts/WayPoints/camera-hud1.png")
				end
				self.cl.hud:setText( "Time", "" )
			else
				self.cl.hud:setImage("COMPASS_BG", "")
				self.cl.hud:setText( "COMPASS_zoom", "" )
				self.cl.hud:setText( "COMPASS_limit", "" )
			end
			local radarDataT = ''
			local compassT = '|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-'
			local compassA = 'N                NE                E                SE                S                SW              W                NW                  '
			local chPos = sm.camera.getPosition() 
			...
			local newCompA = ''
			for i=0,#compassT do
				local charI = math.floor(math.fmod(i-fitAng+3,#compassT))
				newCompT = newCompT .. compassT:sub(charI,charI)
				newCompA = newCompA .. compassA:sub(charI,charI)
			end
			local deg = math.floor(math.deg(yaw))
			if deg ~= math.abs(deg) or deg == 0 then deg = deg + 360 end
			local newStr = newCompT:sub(#newCompT/2+7, #newCompT)
			local newStr2 = newCompT:sub(0, #newCompT/2-7)
			local newCompT = newStr2 .. "              " .. newStr
			local Map, Anim = GetMapMode()
			if not Map then
				self.cl.hud:setText( 'COMPASS_deg', tostring(deg))
				self.cl.hud:setText( 'COMPASS_comp', newCompT )
				self.cl.hud:setText( 'COMPASS_alt', newCompA )
			end
			local posT = ''
