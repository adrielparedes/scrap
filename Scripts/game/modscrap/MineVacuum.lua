...
MineVacuum = class()
MineVacuum.poseWeightCount = 1
MineVacuum.connectionInput = sm.interactable.connectionType.logic
MineVacuum.maxParentCount = 1
MineVacuum.maxChildCount = 0
MineVacuum.fireDelay = 40 -- ticks

local MineVacuumMode = { outgoing = 1, incoming = 2 }
...
function MineVacuum.sv_init( self )
	self:sv_buildPipeNetwork()
	self:sv_updateStates()
end

function MineVacuum.server_onRefresh( self )
	self:sv_init()
end

function MineVacuum.server_onCreate( self )
	self.sv = {}
	...
	if self.sv.storage == nil then
		self.sv.storage = { mode = MineVacuumMode.incoming } -- Default value
...
function MineVacuum.sv_markClientTableAsDirty( self )
	self.sv.dirtyClientTable = true
end

function MineVacuum.sv_markStorageTableAsDirty( self )
	self.sv.dirtyStorageTable = true
	self:sv_markClientTableAsDirty()
end

function MineVacuum.sv_n_toogle( self )
	if self.sv.storage.mode == MineVacuumMode.outgoing then
		self:server_outgoingReset()
		self.sv.storage.mode = MineVacuumMode.incoming
	end
...
function MineVacuum.sv_updateStates( self )

	if self.sv.storage.mode == MineVacuumMode.incoming then
...
function MineVacuum.sv_buildPipeNetwork( self )
	...
	local function fnOnVertex( vertex )
		...
		local authorizedContainers = { sm.uuid.new("2482c8db-ded7-4d88-b1fe-d93f19cd3b31"), sm.uuid.new("be29592a-ef58-4b1d-b18c-895023abd27f") }

		if isAnyOf( vertex.shape:getShapeUuid(), authorizedContainers ) then -- Is authorized
...
function MineVacuum.constructionRayCast( self )
...
function MineVacuum.server_outgoingReload( self, container, item )
	...
	if self.sv.canFire then
		self.sv.fireDelayProgress = MineVacuum.fireDelay
...
function MineVacuum.server_outgoingReset( self )
	self.sv.canFire = false
	self.sv.foundContainer = nil
	self.sv.foundItem = sm.uuid.getNil()
...
function MineVacuum.server_outgoingLoaded( self )
	return self.sv.foundContainer and self.sv.foundItem ~= sm.uuid.getNil()
end

function MineVacuum.server_outgoingShouldReload( self, container, item )
	return self.sv.foundItem ~= item
end

function MineVacuum.server_onFixedUpdate( self )

	local function setMineVacuumState( state, shapes )
	...
	local function setMineVacuumStateOnAllShapes( state )
		if self.sv.client.state ~= state then
			self.sv.client.state = state
			self:sv_markClientTableAsDirty()
		end
	...
	if self.sv.storage.mode == MineVacuumMode.outgoing and #self.sv.connectedContainers > 0 then
		...
		if interfacingWithPackingStation then
			...
			if item == itemUid then
				...
				setMineVacuumState( PipeState.valid, self.sv.connectedContainers[1].shapesOnContainerPath )
			else
				publicData.requestExternalOpen = false
				setMineVacuumState( PipeState.invalid, self.sv.connectedContainers[1].shapesOnContainerPath )
			end
			...
			publicData.requestExternalOpen = false
			setMineVacuumState( PipeState.connected, self.sv.connectedContainers[1].shapesOnContainerPath )
		end
	...
	elseif self.sv.storage.mode == MineVacuumMode.incoming and #self.sv.connectedContainers > 0 then
		...
		for _, result in ipairs(  self.sv.areaTrigger:getContents() ) do
			if sm.exists( result ) then
				if type( result ) == "Harvestable" then
					local getResult = assert(result)
					if getResult.uuid == hvs_farmables_oilgeyser then
						local partUuid = "1147e59d-6940-42b4-840b-07f05054f5e0"
						local amount = 10
						if partUuid and amount then
							...
							if container then
								table.insert( incomingObjects, { container = container, harvestable = result, uuid = partUuid, amount = amount } )
							end
						end
					end
				elseif type( result ) == "AreaTrigger" then
					local userData = result:getUserData()
					if userData and userData.chemical == true then
		...
		if #incomingObjects > 0 then
			...
			setMineVacuumState( PipeState.valid, longestConnection.shapesOnContainerPath )
		else
			setMineVacuumStateOnAllShapes( PipeState.connected )
		end
	end
...
-- Client
function MineVacuum.client_onCreate( self )
	self.cl = {}

	-- Update from onClientDataUpdate
	self.cl.mode = MineVacuumMode.outgoing
	self.cl.pipeNetwork = {}
...
function MineVacuum.client_onClientDataUpdate( self, clientData )
...
function MineVacuum.client_onUpdate( self, dt )
...
function MineVacuum.cl_n_onOutgoingReload( self, data )
...
function MineVacuum.cl_n_onOutgoingFire( self, data )
...
function MineVacuum.cl_n_onIncomingFire( self, data )
...
function MineVacuum.cl_n_onError( self, data )
	self:cl_setOverrideUvIndexFrame( data.shapesOnContainerPath, PipeState.invalid )
end

-- State sets

function MineVacuum.cl_pushEffectTask( self, shapeList, effect )
	self.cl.pipeEffectPlayer:pushEffectTask( shapeList, effect )
end

function MineVacuum.cl_setOverrideUvIndexFrame( self, shapeList, state )
	local shapeMap = {}
	if shapeList then
...
function MineVacuum.cl_setPoseAnimTask( self, name )
...
function MineVacuum.cl_updatePoseAnims( self, dt )
...
function MineVacuum.cl_updateUvIndexFrames( self, dt )
	...
	MineVacuumFrameIndexTable = {
		[MineVacuumMode.outgoing] = {
			[PipeState.off] = 0,
			[PipeState.invalid] = 1,
			[PipeState.connected] = 2,
			[PipeState.valid] = 4
		},
		[MineVacuumMode.incoming] = {
			[PipeState.off] = 0,
			[PipeState.invalid] = 1,
	...
	local vacuumFrameIndex = MineVacuumFrameIndexTable[self.cl.mode][state]
