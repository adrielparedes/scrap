dofile "Libs.lua"
-- BlockLetters.lua --
BlockLetters = class( nil )
BlockLetters.maxParentCount = -1
BlockLetters.maxChildCount = -1
BlockLetters.connectionInput =  sm.interactable.connectionType.power + sm.interactable.connectionType.logic
BlockLetters.connectionOutput = sm.interactable.connectionType.power
BlockLetters.colorNormal = sm.color.new( 0xD8D220ff )
BlockLetters.colorHighlight = sm.color.new( 0xF2EC3Cff )
BlockLetters.poseWeightCount = 1


BlockLetters.power = -1
BlockLetters.buttonwasactive = false
BlockLetters.bin = {
...
BlockLetters.savemodes = {}
for k,v in pairs(BlockLetters.icons) do
   BlockLetters.savemodes[v.uv]=k
end


function BlockLetters.server_onRefresh( self )
	sm.isDev = true
	self:server_onCreate()
end
function BlockLetters.server_onCreate( self )
	local stored = self.storage:load()
	if stored then
		if type(stored) == "number" then
			mpPrint('loading old version data')
			self.power = BlockLetters.savemodes[stored] or 0
			--for k, v in pairs(BlockLetters001) do
			--	self[k] = v
			--end  -- remove janky old compatibility
		elseif type(stored) == "table" then
			self.power = (BlockLetters.savemodes[stored[1]] - 1) or 0
			--version = stored[2]
		end
	else
...
function BlockLetters.server_changemode(self, crouch)
...
function BlockLetters.client_onInteract(self, character, lookAt)
...
function BlockLetters.client_canInteract(self)
...
function BlockLetters.client_playsound(self, sound)
	sm.audio.play(sound, self.shape:getWorldPosition())
end

function BlockLetters.server_onFixedUpdate( self, dt )
...
function BlockLetters.client_onFixedUpdate(self, dt)
