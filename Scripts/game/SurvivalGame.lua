...
function SurvivalGame.server_onCreate( self )
	...
	SetOverWorld(self.sv.saved.overworld)
 end
...
function ForceLoadCells( params )
	SurvivalGame.LoadCells = true
	SurvivalGame.Player = params.player
	SurvivalGame.pos = params.pos
end
...
function SurvivalGame.client_onCreate( self )
	...
	sm.game.bindChatCommand( "/reset-map-mod", { }, "cl_ResetMapMod", "Resets The Map Mod Saved Data" )
	sm.game.bindChatCommand( "/remove-map-mod", { }, "cl_RemoveMapMod", "Removes The Map Mod Saved Data" )
	self.cl = {}
	self.cl.time = {}
...
function SurvivalGame.cl_ResetMapMod( self )
	local survival_player = SurvivalPlayer()
	survival_player:reset_map_data()
	self.network:sendToServer("MessageAll", "#ff0000RESETING MAP MOD\nPlease Wait For The Map Mod To Fully Re-Save All Of Your Data#ffffff")
end

function SurvivalGame.cl_RemoveMapMod( self )
	local survival_player = SurvivalPlayer()
	survival_player:remove_map_data()
	self.network:sendToServer("MessageAll", "#ff0000REMOVING MAP MOD\nPlease Wait For The Map Mod To Fully Delete Itself#ffffff")
end

function SurvivalGame.tp_player( self, params )
	SurvivalGame.Tp_Params = params
end

function SurvivalGame.sv_tp_player( self, params )
	self.sv.saved.overworld:loadCell( math.floor( params.pos.x/64 ), math.floor( params.pos.y/64 ), params.player, "sv_recreatePlayerCharacter", params )
	SurvivalGame.Tp_Params = nil
end

function SurvivalGame.MessageAll( self, message )
	self.network:sendToClients( "client_showMessage", message )
end
...
function SurvivalGame.server_onFixedUpdate( self, timeStep )
	SetOverWorld(self.sv.saved.overworld)
	if SurvivalGame.Tp_Params ~= nil then
		self:sv_tp_player(SurvivalGame.Tp_Params)
	end
	if SurvivalGame.LoadCells == true then
		print("Force Loading Cell: "..tostring( math.floor( SurvivalGame.pos.x/64 )..", ".. math.floor( SurvivalGame.pos.y/64)))
